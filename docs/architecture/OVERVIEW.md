...
| **Tools / MCP** | Executable interfaces and their transport boundary `[P§22]` | Deciding *whether* something should be done | Skills, orchestration | ADR-0005 (**Accepted** 2026-09-24) | Partial — `cv_agent/tools/` implements the generic `ToolSpec`/`ToolInvoker`/`ToolRegistry`/`ToolExecutor` boundary, including invoker-registration-generation protection and the `ToolOutcome`/`ToolResult` trust-boundary split (71 tests). Zero `ToolSpec`s/`ToolInvoker`s registered, no MCP SDK selected — the boundary only, no real tool yet. `cv_agent/execution/` (ADR-0009) remains the separate, real, narrower skill-execution boundary — unaffected and not superseded by ADR-0005. |
| **Knowledge / RAG** | Retrieval, indexing, provenance, freshness, credibility weighting `[P§16]`, `[P§18]`, `[P§19]` | Reasoning, decision-making, execution | Reasoning (read-only) | ADR-0006 (**Accepted** 2026-09-24) | Partial — `cv_agent/knowledge/` implements the provenance-gated `KnowledgeItem`/`Provenance`/`KnowledgeStore`/`assemble_context()` contract (fail-closed construction, deterministic retrieval and tie-break, source-class evidence weighting per `docs/RESEARCH_POLICY.md`), 55 tests. Only an in-memory reference store ships — no durable backend, no acquisition mechanism (no web fetching yet), no embeddings/ranking model. |
| **Web research** | Acquisition of current external information + source classification `[P§17]`, `[P§18]` | Long-term storage semantics (that is Knowledge) | Knowledge layer | ADR-0006 (not yet written for acquisition — storage/provenance half only) | Planned — no code. `cv_agent/knowledge/` (above) accepts already-extracted items; nothing yet calls it with real research output. |
...
| **Experiment ledger** | Reproducible experiment metadata and results `[P§25]` | Running experiments | Training, evaluation | ADR-0011 (**Accepted** 2026-09-24; `ADR-0004` explicitly scoped it out) | Partial — `cv_agent/experiments/` implements the full `[P§25]` schema as a typed, fail-closed `ExperimentRecord` behind an `ExperimentLedger` protocol, SQLite-backed (`OPEN_QUESTIONS.md` Q16, resolved; own `experiments.sqlite`, 90 tests). `docs/state/EXPERIMENTS.md` remains the human-facing schema authority. Not wired into `CVAgent`/the CLI; zero real rows — no training/evaluation subsystem exists to write any. |
| **Dataset subsystem** | Manifests, versions, lineage, splits, leakage checks `[P§26]` | Annotation UI, model logic | Training, evaluation | ADR-0012 (**Accepted** 2026-09-24) | Partial — `cv_agent/datasets/` implements the storage-agnostic core: immutable `DatasetManifest` (a dataset version), recorded splits, deterministic leakage checks over supplied attributes/hashes, `DatasetStore` protocol + in-memory reference (88 tests). No storage/versioning backend (Q10 open), no hash computation, no acquisition, not wired into anything. |
