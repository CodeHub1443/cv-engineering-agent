...
|---|---|---|---|---|---|
| **LLM Gateway** | Provider abstraction, model selection by task class & cost, retries/limits, token accounting `[P§20]` | Prompts as business logic, orchestration, knowledge, tool execution | Reasoning nodes only | ADR-0002 (not yet written) | Planned — `cv_agent/llm/registry.py` exists but registers only `FakeLLMProvider`; no real provider adapter, no ADR-0002. |
| **Reasoning nodes** | Judgment: problem framing, task decomposition, architecture choice, diagnosis `[P§4]`, `[P§5]` | Provider details, retrieval mechanics, tool implementations, workflow control flow | Gateway, knowledge (read), capability registry | ADR-0008 | Partial — `cv_agent/requirements/` performs deterministic (non-LLM) task decomposition and field elicitation; no real LLM judgment yet (LLM Gateway above is Planned). |
