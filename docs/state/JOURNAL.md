...
## 2026-09-24 — PR #52/#54 merged; ADR-0006 Knowledge/Context boundary drafted, self-reviewed, and implemented (branch `feature/claude/adr-0006-knowledge-context-boundary`, issue #55)

**Did:** Two-part session, both parts under the owner's explicit continuous-execution instruction to merge-and-continue rather than stop at each PR boundary. **Part 1:** validated PR #52 (ADR-0002) exhaustively (model string, env-only credentials, no fallback, mock default, no execution/approval bypass, Q3/Q6/Q19 still open, 646/646 tests, ruff/mypy/diff-check clean, CI green) and, on the owner's explicit "then merge" instruction, squash-merged it (`ae96b9f`) — CLAUDE.md §6.3 reserves merging to the owner by default; this was a one-time, explicitly granted override, not a standing policy change. Rebased `feature/claude/adr-0005-tool-mcp-boundary` onto the new `main` using `git rebase --onto origin/main 1853e0a ...` (not a plain `git rebase`, which produced add/add conflicts trying to replay the now-squashed ADR-0002 commits a second time), verified the resulting diff was pure ADR-0005, validated (717/717, clean), force-pushed, and squash-merged PR #54 (`d730838`) under the same owner authorization. **Part 2:** re-audited the repository against `main` @ `d730838` (`ROADMAP.md` actually defines 9 phases, 0–8, not the "20" the task framing assumed — corrected inline), classified each phase, and re-triaged Q2/Q3/Q5/Q6/Q10/Q16/Q19/Q23 against what they currently block. Selected ADR-0006 (Knowledge/Context boundary) as the next milestone by direct appeal to `OVERVIEW.md`'s own declared build sequence (step 6, right after step 5's tool boundary) — not by default assumption — after showing the two earlier-sequence gaps (experiment-ledger persistence, a durable checkpointer) are both blocked by open owner decisions (Q16, Q3), same as the Phase-3/4 gaps (Q23). Drafted ADR-0006, self-reviewed it inline against the same five criteria ADR-0005's independent review used (documented in the ADR's own §9), then implemented `cv_agent/knowledge/`: `Provenance`/`KnowledgeItem` (mandatory, `__post_init__`-validated provenance — an item without it cannot be constructed, let alone stored), a fixed `SourceClass → EvidenceWeight` mapping taken verbatim from `docs/RESEARCH_POLICY.md`'s table (not a per-item override, so a `professional_post`/LinkedIn source can never claim `"high"` weight), `KnowledgeStore` protocol + `InMemoryKnowledgeStore` (deliberately no durable backend — mirrors ADR-0005's "boundary only" scoping and avoids inventing Q16's still-open equivalent decision for this layer), and deterministic `assemble_context()` (exact tag/class/staleness filtering, a fully explicit tie-break order, no embeddings/ranking model). 55 new tests (`tests/test_knowledge.py`), full suite 717 → 772.

**Why:** `[P§16]`–`[P§19]` (knowledge/research canon), `[P§29.3]` (evidence over hype), `[P§34]` (boundary test), `[P§35]` (no silent invention). ADR-0006 per `OVERVIEW.md` design-sequence step 6; issue #55.

**Broke:** One test-data bug (not implementation) in the tie-break test: two items dated in January were checked against an `as_of` in June with the default 90-day staleness horizon, so they were filtered out as stale before the tie-break logic ever ran, producing a 1-item result instead of 3. Fixed by giving those two items an explicit 365-day horizon — the test was about ordering, not staleness, and needed to say so. Caught immediately by the first test run, never shipped. A first draft of `assemble_context()`'s sort key also had dead leftover code from an editing mistake (`(...)[:0] or (...)`) — functionally correct by accident but unreadable; rewritten cleanly with `date.toordinal()` negation before any test ran against it.

**Learned:** The `ExecutionBinding`/`ToolSpec` fail-closed `__post_init__` validation pattern transferred directly to `KnowledgeItem`/`Provenance` with no adaptation — a third confirmation (after ADR-0005) that "validate at construction, not at a later checkpoint" is this codebase's settled idiom for anything with an integrity requirement. Proving a milestone selection from the repository (per this session's own instruction) is stronger than assuming the roadmap's stated next phase — the experiment-ledger and checkpointer gaps looked like they came first in `OVERVIEW.md`'s sequence, and only checking each against `OPEN_QUESTIONS.md` showed they were blocked while the knowledge boundary was not.

**Left open:** No acquisition mechanism (web research, an NVIDIA-doc tool, a first real `ToolInvoker`) exists to populate `KnowledgeStore` with real items — this ADR ships the contract, not a producer. No durable backend chosen (mirrors Q16). `cv_agent.knowledge` is not wired into any reasoning node, `CVAgent`, or `LangGraph`. `OPEN_QUESTIONS.md` untouched; Q2/Q3/Q5/Q6/Q10/Q16/Q19/Q23 all remain unresolved. Per CLAUDE.md §6.3 (owner reviews and merges PRs) and this session's own instruction ("if the repository explicitly permits the agent to merge... otherwise stop at the owner-review boundary"), this PR was **not** self-merged — no fresh "then merge" instruction was given for it, unlike PR #52/#54. PR review/merge pending.

---

## 2026-09-24 — PR #56 merged; Q16 resolved (SQLite) and the Experiment Ledger implemented (branch `feature/claude/q16-sqlite-experiment-ledger`, issue #57)

**Did:** Merged PR #56 (ADR-0006) as `10fb89f` on the owner's explicit authorization, after re-confirming the head SHA, CI, and diff were unchanged since review. Then, on the owner's explicit resolution of Q16 (SQLite), read `cv_agent/memory/`'s `models.py`/`store.py`/`sqlite_store.py`, ADR-0004, `tests/test_memory.py`, and `docs/state/EXPERIMENTS.md` in full before designing anything, and implemented `cv_agent/experiments/` by reusing that pattern exactly: `ExperimentRecord` (every field name verbatim from `EXPERIMENTS.md`'s schema table), an `ExperimentLedger` `Protocol`, `open_ledger()`/`default_db_path()`, and `SqliteExperimentLedger`, in a separate `experiments.sqlite` file. Wrote ADR-0011 (took the next free number, the same "numbers follow implementation order" practice `OVERVIEW.md` documents, even though `ROADMAP.md` Phase 6's aspirational table had nominally reserved that number). Updated `EXPERIMENTS.md` with a relationship note (it stays the human-facing schema authority) and fixed its stale `Q8` pointer to `Q16`; struck through Q16 in `OPEN_QUESTIONS.md` (kept, not deleted); D-038. 90 new tests, suite 772 → 862.

**Why:** `[P§25]`, `[P§29.5]` (reproducibility), `[P§34]` (boundary test), `[P§35]`; ROADMAP Phase 1 exit test #4.

**Broke:** One false positive against a *pre-existing* test I did not write: `tests/test_memory.py::TestArchitectureBoundary` scans **every** package under `cv_agent/` (not only `cv_agent/memory/`) for the literal substring `"import sqlite3"` and exempts only files literally named `sqlite_store.py`. My `cv_agent/experiments/store.py` docstring said the concrete backend was "the only module... allowed to import sqlite3" — prose, not an import, but a substring match. Fixed by rewording my docstring; deliberately did **not** touch that test, which is outside this task's scope and is correctly a repo-wide rule. Same class of mistake as the two false-positive boundary tests written for `cv_agent/tools/` earlier this session — checking for a substring instead of an actual import statement — except this time the too-crude check was pre-existing, not mine, so the fix belongs on the side that can reasonably change (my wording).

**Learned:** `EXPERIMENTS.md` rule 1 ("never edit a row after the run completes") is a *third* mutability model, distinct from both `cv_agent/memory/` precedents (immutable-append revisions; unrestricted-upsert sessions): upsertable while non-terminal, locked once terminal. It has no precedent to copy, so it needed its own tests (each of the three terminal statuses individually, plus proof the original terminal row survives a rejected write untouched). Three schema rows (`hardware`, `latency`, `memory`) describe two values under one field name in prose only — a real ambiguity in the source table, resolved by nested value objects under the same field name rather than either splitting (renaming) or collapsing (dropping); recorded in ADR-0011 §3 as an interpretation, not hidden.

**Left open:** Not wired into `CVAgent`/the CLI — nothing calls `cv_agent.experiments` yet, and no training/evaluation subsystem exists to write real rows. `dataset_version`/`model` validated as well-formed strings only; a real manifest cross-check awaits a dataset subsystem (Phase 5, Q10). Q3 and Q23 remain separate, unresolved owner decisions — not implied by Q16's answer. PR review/merge pending (no fresh merge authorization given for this PR).

---

## 2026-09-24 — PR #58 merged; Phase 5a Dataset Core implemented (branch `feature/claude/dataset-core`, issue #59)

**Did:** On the owner's explicit instruction, verified PR #58 (head `7c05e83`, CI green, same 13 files reviewed in the audit, clean tree) and squash-merged it as `44539c3` (CI on the merged commit: success). Earlier in the session the owner's claim that #58 was merged was checked against `gh`/`origin/main`, found false, and work stopped until the merge was actually authorized. Then created issue #59 and branch `feature/claude/dataset-core` from the fresh `origin/main`, wrote ADR-0012, wrote `tests/test_datasets.py` first (shown failing: `ModuleNotFoundError`), and implemented `cv_agent/datasets/` (stdlib only): `models.py` (value types), `leakage.py` (`check_leakage`, report types), `manifest.py` (`DatasetManifest`), `store.py` (`DatasetStore` protocol, `DatasetStoreError`), `memory_store.py` (`InMemoryDatasetStore`). `DatasetManifest` is the dataset version — `docs/DATA.md`'s own terms — and cannot be constructed with a failing leakage check; the report is computed in the constructor and recorded in it.

**Why:** ROADMAP Phase 5a; `[P§26]`, `docs/DATA.md`; OVERVIEW design-sequence step 4; the audit's finding that the dataset core was the only next milestone touching none of Q2/Q3/Q6/Q19/Q23.

**Broke:** Nothing shipped broken. One test-fixture mistake caught by the first run (dropping the `test` split left the default `time_order` naming it, so a different, earlier error fired). All 88 tests passed on the first full run, which is exactly when a suite is most likely to be vacuous — so 11 manual mutation checks were run (camera check off, `>=`→`>` time boundary, `<=`→`<` Hamming, subject always n/a, segment straddle ignored, mixing always allowed, manifest ignoring a failed report, store overwrite allowed, store ignoring lineage parent, missing hash passing, an unfrozen item); each was killed by at least one test and every file was restored byte-for-byte.

**Learned:** `docs/DATA.md` specifies five leakage checks and a manifest field list but no per-item schema and no way to state which goal applies — the ADR names each such gap and resolves it fail-closed rather than leaving it implicit. "Recorded in the manifest" plus "every dataset has a manifest" makes a single immutable type stronger than a version/manifest pair: there is no place to forge or omit a report. The strictness is a real cost (every item needs a supplied hash and attributes) and is recorded as a revisit trigger, not hidden.

**Left open:** Q10 (backend) — deliberately not decided; no persistence format exists. No perceptual-hash producer. No baseline workflow (Phase 5b needs Q4/Q2 and a real execution mechanism). Not wired into `CVAgent` or the ledger (ledger `dataset_version` cross-check unbuilt). Diff exceeds `CLAUDE.md` §6's ~400-line guideline (~1,030 source, ~790 tests) because the owner scoped the milestone as one unit; noted in the PR. PR review/merge pending — no merge authorization for it.

---

## 2026-09-24 — Owner decisions Q2/Q4/Q5 recorded; Phase 5b prepared (branch `docs/claude/q2-q4-q5-decisions`, issue #61)

**Did:** After PR #60 (Dataset Core) merged as `e6cff6e`, recorded three owner decisions as D-040/D-041/D-042 and struck Q2, Q4, Q5 through in place in `OPEN_QUESTIONS.md` (the Q16/Q22 convention), each stating what it does *not* decide. Q2: V1 runs on a local Linux/NVIDIA GPU host with the Agent/controller and CV workloads as separate processes (controlled jobs). Q4: V1 validates on a controlled reference project before customer projects. Q5: V1 uses the existing Skill/CLI mechanism behind the `ToolInvoker`/execution boundaries; MCP SDK deferred, transport-agnostic. Updated ROADMAP (Phase 2's Q5 sentence, Phase 5 "Phase 5b preparation" paragraph) and rewrote STATUS. Added two explicit new questions found while recording: Q24 (which reference project) and Q25 (which host/GPU, and where the controller runs). No source or test changed.

**Why:** `CLAUDE.md` §7 (rolling state) and §1 (an unanswered question is recorded, not invented); the previous audit's finding that Phase 5b was gated on Q2/Q4/Q5 and a real execution mechanism.

**Broke:** Nothing. Decision on ADRs: none written — under `CLAUDE.md` §5 none of the three introduces a module, changes a layer interface, chooses a dependency or changes state shape; Q5 is already consistent with the accepted ADR-0005 and `spec/06`, and Q2 constrains the *future* training-execution ADR rather than amending an existing one. Historical ADR sentences that call Q2/Q5 open (ADR-0003 §1, ADR-0005 §1/§8/§10) were true when written and were left unedited, per this repo's append-only practice.

**Learned:** The three answers each leave a gap Phase 5b cannot skip: Q4 did not name the reference project (Q24); Q2 did not name the host or say whether the controller shares it, and this repository's sessions run on Windows while `docs/APPROVALS.md` forbids Linux-specific commands on another platform (Q25); Q5 chose a mechanism but two boundaries exist for it (ADR-0009 skill bindings and ADR-0005 `ToolInvoker`), and which one hosts the first real mechanism remains open under ADR-0005 §8(c) — an architecture decision for the ADR that builds it, not a new owner question.

**Left open:** Q3, Q6, Q10, Q19, Q23 untouched. Q24/Q25 need owner answers. No baseline execution, training, ToolInvoker, MCP, binding, threshold, estimator, checkpointer or dataset backend was built. Python 3.10 `Z`-timestamp behavior and ADR-0012 wording remain deferred follow-ups. PR review/merge pending.
